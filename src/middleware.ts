...
export async function middleware(request: NextRequest) {
  // 🚩 CHANGE 1: start with `NextResponse.next()`, not a redirect
  ...
  // ✅ If correct country code in URL + cookie exists → just continue
  if (urlHasCountryCode && cacheIdCookie) {
    return response
  }

  // ✅ If URL has country code but cookie is missing → set cookie, but no redirect loop
  if (urlHasCountryCode && !cacheIdCookie) {
  ...
  // 🚩 CHANGE 2: only redirect when missing a country code in the path
